		...
		<c:if test="<%= deltaConfigurable %>">
			<div class="dropdown pagination-items-per-page">
				<a class="dropdown-toggle page-link" data-toggle="dropdown" href="javascript:;">
					<liferay-ui:message arguments="<%= delta %>" key="x-entries" /><span class="sr-only">&nbsp;<liferay-ui:message key="per-page" /></span>
					...
					%>
						...
						<li>
							<a href="<%= HtmlUtil.escapeHREF(curDeltaURL) %>" onClick="<%= forcePost ? _getOnClick(namespace, deltaParam, curDelta) : "" %>">
								<%= String.valueOf(curDelta) %><span class="sr-only">&nbsp;<liferay-ui:message key="entries-per-page" /></span>
							</a>
						</li>
				...
				<c:otherwise>
					...
					%>
						...
						<li class="<%= ((cur - 3) > 1) ? "" : "page-item" %>">
							<a class="<%= ((cur - 3) > 1) ? "dropdown-icon" : "dropdown-icon page-link" %>" href="<%= _getHREF(formName, namespace + curParam, i, jsCall, url, urlAnchor) %>" onclick="<%= forcePost ? _getOnClick(namespace, curParam, i) : "" %>"><span class="sr-only"><liferay-ui:message key="page" />&nbsp</span><%= i %></a>
						</li>
					...
					%>
						...
						<li class="<%= ((cur + 3) < pages) ? "" : "page-item" %>">
							<a class="<%= ((cur + 3) < pages) ? "dropdown-icon" : "dropdown-icon page-link" %>" href="<%= _getHREF(formName, namespace + curParam, i, jsCall, url, urlAnchor) %>" onclick="<%= forcePost ? _getOnClick(namespace, curParam, i) : "" %>"><span class="sr-only"><liferay-ui:message key="page" />&nbsp;</span><%= i %></a>
						</li>
