...
public class TextEmbeddingContentHelper<T extends BaseModel<T>> {
	...
	public void appendToAll(String value) {
		...
		for (StringBundler languageContentSB :
				_languageIdToContentSBMap.values()) {

			_append(languageContentSB, value);
		}
	}
	...
	public void contribute(Document document) {
		...
		for (String languageId :
			...
			StringBundler contentSB = _languageIdToContentSBMap.get(languageId);

			if (contentSB.length() <= 0) {
				contentSB = _languageIdToContentSBMap.get(_defaultLanguageId);
			}
	...
	public Map<String, String> getLanguageIdToContentMap() {
		Map<String, String> languageIdToContentMap = new HashMap<>();
