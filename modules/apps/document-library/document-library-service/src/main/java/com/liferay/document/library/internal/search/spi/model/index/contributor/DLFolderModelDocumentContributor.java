...
public class DLFolderModelDocumentContributor
	...
	private boolean _changingPermissions(Document document, DLFolder dlFolder) {
		...
		List<String> indexedGroupRoles = indexedDocument.getStrings(
			"groupRoleId");
		...
		try {
			roles = _resourcePermissionLocalService.getDynamicInheritanceRoles(
				dlFolder.getCompanyId(),
			...
			_log.error("Unable to retrieve roles for DLFolder " +
