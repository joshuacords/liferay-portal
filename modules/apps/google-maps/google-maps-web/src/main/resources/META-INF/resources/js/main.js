		...
		var GoogleMaps = A.Component.create({
			...
			prototype: {
				...
				bindUI: function() {
					...
					var getDirectionsButtonNode = instance.byId(
						'getDirectionsButton'
					);

					if (getDirectionsButtonNode) {
						eventHandles.push(
							getDirectionsButtonNode.on(
								STR_CLICK,
					...
					var directionsAddressNode = instance.byId(
						STR_DIRECTION_ADDRESS
					);

					if (directionsAddressNode) {
						eventHandles.push(
							directionsAddressNode.on(
								STR_KEY,
								A.bind(
									instance._onDirectionsAddressKeyDown,
									instance
								),
								KEY_DOWN_ENTER
							)
					...
					if (mapAddressNode) {
						eventHandles.push(
							mapAddressNode.on(
								STR_KEY,
								A.bind(instance._onMapAddressKeyDown, instance),
								KEY_DOWN_ENTER
							)
						);
					}
					...
					if (openLinkNode) {
						eventHandles.push(
							openLinkNode.on(
								STR_CLICK,
								instance._onOpenInGoogleMapsClick,
								instance
							)
						);
					}
					...
					eventHandles.push(
						Liferay.on(
							instance.get('portletId') + ':portletRefreshed',
				...
				_attachInstructionText: function(marker, text) {
					...
					google.maps.event.addListener(
						marker,
						STR_CLICK,
						A.rbind(instance._onMarkerClick, instance, marker, text)
					);
				},
				...
				_getDirections: function() {
					...
					var directionsAddress = instance
						.byId(STR_DIRECTION_ADDRESS)
						.val();
					...
					instance._directionsService.route(
						request,
						A.rbind(instance._onRoute, instance, directionsAddress)
					);
				},
				...
				_initGoogleMaps: function() {
					...
					googleMapsURL =
						...
						instance.get('languageId') +
						'&callback=Liferay.GOOGLE_MAPS.onGoogleMapsLoaded' +
						'&key=' +
						instance.get('apiKey');
				...
				_isDirectionFilled: function() {
					...
					return (
						instance.get(STR_DIRECTION_ADDRESS) &&
						instance.byId(STR_DIRECTION_ADDRESS).val()
					);
				},
				...
				_onOpenInGoogleMapsClick: function(event) {
					...
					var url =
						'http://maps.google.com/maps?q=' + encodedMapAddress;
					...
					if (directionsAddress) {
						url =
							...
							encodedMapAddress +
							'+to+' +
							encodeURIComponent(directionsAddress);
					}
				...
				_renderMap: function() {
					...
					instance._map = new googleMaps.Map(
						instance.byId('map').getDOMNode(),
						mapParams
					);
