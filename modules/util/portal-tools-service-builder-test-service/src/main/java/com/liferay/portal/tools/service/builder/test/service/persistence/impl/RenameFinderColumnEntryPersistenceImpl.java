...
public class RenameFinderColumnEntryPersistenceImpl
	 ...
	 *
	 * @param renamedColumn the renamed column
	 ...
	 */
	@Override
	public RenameFinderColumnEntry findByColumnToRename(String renamedColumn)
	...
	/**
	 * Returns the rename finder column entry where renamedColumn = &#63; or returns <code>null</code> if it could not be found. Uses the finder cache.
	 *
	 * @param renamedColumn the renamed column
	 ...
	 */
	@Override
	public RenameFinderColumnEntry fetchByColumnToRename(String renamedColumn) {
		return fetchByColumnToRename(renamedColumn, true);
	}

	/**
	 * Returns the rename finder column entry where renamedColumn = &#63; or returns <code>null</code> if it could not be found, optionally using the finder cache.
	 *
	 * @param renamedColumn the renamed column
	...
	public RenameFinderColumnEntry fetchByColumnToRename(
		String renamedColumn, boolean useFinderCache) {
		...
		if (useFinderCache) {
			finderArgs = new Object[] {renamedColumn};
		}
		...
		if (result instanceof RenameFinderColumnEntry) {
			...
			if (!Objects.equals(
					renamedColumn,
					renameFinderColumnEntry.getRenamedColumn())) {
	...
	/**
	 * Removes the rename finder column entry where renamedColumn = &#63; from the database.
	 *
	 * @param renamedColumn the renamed column
	 * @return the rename finder column entry that was removed
	 */
	@Override
	public RenameFinderColumnEntry removeByColumnToRename(String renamedColumn)
		throws NoSuchRenameFinderColumnEntryException {
	...
	/**
	 * Returns the number of rename finder column entries where renamedColumn = &#63;.
	 *
	 * @param renamedColumn the renamed column
	...
	private static final String _FINDER_COLUMN_COLUMNTORENAME_RENAMEDCOLUMN_2 =
		"renameFinderColumnEntry.renamedColumn = ?";

	private static final String _FINDER_COLUMN_COLUMNTORENAME_RENAMEDCOLUMN_3 =
		"(renameFinderColumnEntry.renamedColumn IS NULL OR renameFinderColumnEntry.renamedColumn = '')";
	...
	public RenameFinderColumnEntryPersistenceImpl() {
		...
		setEntityCacheEnabled(
			RenameFinderColumnEntryModelImpl.ENTITY_CACHE_ENABLED);
	}
	...
	public void cacheResult(RenameFinderColumnEntry renameFinderColumnEntry) {
		...
		finderCache.putResult(
			...
			new Object[] {renameFinderColumnEntry.getRenamedColumn()},
	...
	public void clearCache(RenameFinderColumnEntry renameFinderColumnEntry) {
		entityCache.removeResult(
			RenameFinderColumnEntryModelImpl.ENTITY_CACHE_ENABLED,
			RenameFinderColumnEntryImpl.class,
			renameFinderColumnEntry.getPrimaryKey());

		finderCache.clearCache(FINDER_CLASS_NAME_LIST_WITH_PAGINATION);
		finderCache.clearCache(FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION);

		clearUniqueFindersCache(
			(RenameFinderColumnEntryModelImpl)renameFinderColumnEntry, true);
	}
	...
	protected void cacheUniqueFindersCache(
		...
		Object[] args = new Object[] {
			renameFinderColumnEntryModelImpl.getRenamedColumn()
		};

		finderCache.putResult(
			_finderPathCountByColumnToRename, args, Long.valueOf(1), false);
		finderCache.putResult(
			_finderPathFetchByColumnToRename, args,
			renameFinderColumnEntryModelImpl, false);
	}
	...
	protected void clearUniqueFindersCache(
		...
		if (clearCurrent) {
			...
			finderCache.removeResult(_finderPathCountByColumnToRename, args);
			finderCache.removeResult(_finderPathFetchByColumnToRename, args);
		}

		if ((renameFinderColumnEntryModelImpl.getColumnBitmask() &
			 _finderPathFetchByColumnToRename.getColumnBitmask()) != 0) {

			Object[] args = new Object[] {
				renameFinderColumnEntryModelImpl.getOriginalRenamedColumn()
			};
	...
	public RenameFinderColumnEntry updateImpl(
		...
		if (!RenameFinderColumnEntryModelImpl.COLUMN_BITMASK_ENABLED) {
			finderCache.clearCache(FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION);
		}
		else if (isNew) {
		...
		clearUniqueFindersCache(renameFinderColumnEntryModelImpl, false);
	...
	public void afterPropertiesSet() {
		...
		_finderPathWithPaginationFindAll = new FinderPath(
			RenameFinderColumnEntryModelImpl.ENTITY_CACHE_ENABLED,
			RenameFinderColumnEntryModelImpl.FINDER_CACHE_ENABLED,
			RenameFinderColumnEntryImpl.class,
		...
		_finderPathCountAll = new FinderPath(
			RenameFinderColumnEntryModelImpl.ENTITY_CACHE_ENABLED,
			RenameFinderColumnEntryModelImpl.FINDER_CACHE_ENABLED, Long.class,
		...
		_finderPathFetchByColumnToRename = new FinderPath(
			RenameFinderColumnEntryModelImpl.ENTITY_CACHE_ENABLED,
			RenameFinderColumnEntryModelImpl.FINDER_CACHE_ENABLED,
			RenameFinderColumnEntryImpl.class, FINDER_CLASS_NAME_ENTITY,
			"fetchByColumnToRename", new String[] {String.class.getName()},
			RenameFinderColumnEntryModelImpl.RENAMEDCOLUMN_COLUMN_BITMASK);
