...
public class ERCGroupEntryPersistenceTest {
	...
	public void testResetOriginalValues() throws Exception {
		...
		Assert.assertEquals(
			existingERCGroupEntry.getUuid(),
			ReflectionTestUtil.invoke(
				existingERCGroupEntry, "getOriginalUuid", new Class<?>[0]));
		Assert.assertEquals(
			Long.valueOf(existingERCGroupEntry.getGroupId()),
			ReflectionTestUtil.<Long>invoke(
				existingERCGroupEntry, "getOriginalGroupId", new Class<?>[0]));
		...
		Assert.assertEquals(
			existingERCGroupEntry.getExternalReferenceCode(),
			ReflectionTestUtil.invoke(
				existingERCGroupEntry, "getOriginalExternalReferenceCode",
				new Class<?>[0]));
	}
