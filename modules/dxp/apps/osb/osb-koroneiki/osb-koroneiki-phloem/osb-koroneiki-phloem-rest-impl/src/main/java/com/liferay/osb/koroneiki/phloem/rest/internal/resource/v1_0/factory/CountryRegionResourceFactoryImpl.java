...
public class CountryRegionResourceFactoryImpl
	...
	@Override
	public CountryRegionResource.Builder create() {
		return new CountryRegionResource.Builder() {
			...
			@Override
			public CountryRegionResource.Builder checkPermissions(
			...
			@Override
			public CountryRegionResource.Builder httpServletRequest(
			...
			@Override
			public CountryRegionResource.Builder httpServletResponse(
			...
			@Override
			public CountryRegionResource.Builder preferredLocale(
			...
			@Override
			public CountryRegionResource.Builder user(User user) {
				_user = user;
	...
	private static Function<InvocationHandler, CountryRegionResource>
		...
		Class<?> proxyClass = ProxyUtil.getProxyClass(
			CountryRegionResource.class.getClassLoader(),
			CountryRegionResource.class);

		try {
			Constructor<CountryRegionResource> constructor =
				(Constructor<CountryRegionResource>)proxyClass.getConstructor(
	...
	private Object _invoke(
		...
		countryRegionResource.setContextAcceptLanguage(
			new AcceptLanguageImpl(httpServletRequest, preferredLocale, user));

		Company company = _companyLocalService.getCompany(user.getCompanyId());

		countryRegionResource.setContextCompany(company);

		countryRegionResource.setContextHttpServletRequest(httpServletRequest);
		countryRegionResource.setContextHttpServletResponse(
			httpServletResponse);
		countryRegionResource.setContextUser(user);
		countryRegionResource.setExpressionConvert(_expressionConvert);
		countryRegionResource.setFilterParserProvider(_filterParserProvider);
		countryRegionResource.setGroupLocalService(_groupLocalService);
		countryRegionResource.setResourceActionLocalService(
		...
		try {
			...
			_componentServiceObjects.ungetService(countryRegionResource);
