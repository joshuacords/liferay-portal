...
import com.liferay.portal.search.tuning.blueprints.constants.json.keys.aggregation.pipeline.AvgBucketAggregationBodyConfigurationKeys;
...
public class AvgBucketAggregationTranslator implements AggregationTranslator {
	...
	public Optional<AggregationWrapper> translate(
		...
		if (!BlueprintJSONValidationUtil.validateRequiredFieldsPresent(getClass().getName(),
				jsonObject, messages,
				AvgBucketAggregationBodyConfigurationKeys.BUCKETS_PATH.
					getJsonKey())) {
		...
		_setterHelper.setStringValue(
			jsonObject,
			AvgBucketAggregationBodyConfigurationKeys.FORMAT.getJsonKey(),
