...
import com.liferay.portal.search.tuning.blueprints.constants.json.keys.aggregation.bucket.RangeAggregationBodyConfigurationKeys;
...
public class RangeAggregationTranslator implements AggregationTranslator {
	...
	public Optional<AggregationWrapper> translate(
		...
		if (!BlueprintJSONValidationUtil.validateRequiredFieldsPresent(getClass().getName(),
				jsonObject, messages,
				RangeAggregationBodyConfigurationKeys.FIELD.getJsonKey())) {
		...
		_setterHelper.setStringValue(
			jsonObject,
			RangeAggregationBodyConfigurationKeys.FORMAT.getJsonKey(),
			aggregation::setFormat);

		_setterHelper.setBooleanValue(
			jsonObject,
			RangeAggregationBodyConfigurationKeys.KEYED.getJsonKey(),
			aggregation::setKeyed);

		_setterHelper.setStringValue(
			jsonObject,
			RangeAggregationBodyConfigurationKeys.MISSING.getJsonKey(),
