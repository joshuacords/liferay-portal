 ...
 */
@Component(
	property = "resource.locator.key=/portal-workflow-metrics/v1.0/AssigneeUser",
	service = AssigneeUserResource.Factory.class
)
@Generated("")
public class AssigneeUserResourceFactoryImpl
	...
	public AssigneeUserResource.Builder create() {
		return new AssigneeUserResource.Builder() {
			...
			public AssigneeUserResource build() {
				...
				return _assigneeUserResourceProxyProviderFunction.apply(
			...
			@Override
			public AssigneeUserResource.Builder httpServletResponse(
			...
			@Override
			public AssigneeUserResource.Builder uriInfo(UriInfo uriInfo) {
				_uriInfo = uriInfo;
	...
	private static Function<InvocationHandler, AssigneeUserResource>
		_getProxyProviderFunction() {

		Class<?> proxyClass = ProxyUtil.getProxyClass(
			AssigneeUserResource.class.getClassLoader(),
			AssigneeUserResource.class);

		try {
			Constructor<AssigneeUserResource> constructor =
				(Constructor<AssigneeUserResource>)proxyClass.getConstructor(
	...
	private Object _invoke(
		...
		assigneeUserResource.setContextHttpServletResponse(httpServletResponse);
		assigneeUserResource.setContextUriInfo(uriInfo);
		assigneeUserResource.setContextUser(user);
		assigneeUserResource.setExpressionConvert(_expressionConvert);
		assigneeUserResource.setFilterParserProvider(_filterParserProvider);
		assigneeUserResource.setGroupLocalService(_groupLocalService);
		assigneeUserResource.setResourceActionLocalService(
			_resourceActionLocalService);
		assigneeUserResource.setResourcePermissionLocalService(
			_resourcePermissionLocalService);
		assigneeUserResource.setRoleLocalService(_roleLocalService);
		assigneeUserResource.setSortParserProvider(_sortParserProvider);
	...
	private static final Function<InvocationHandler, AssigneeUserResource>
		_assigneeUserResourceProxyProviderFunction =
			_getProxyProviderFunction();
